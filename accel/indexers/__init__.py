...
def _base_indexes_exist(index_dir: Path) -> bool:
    return all(
        base_path_for_kind(index_dir, kind).exists() for kind in INDEX_KEY_FIELDS
    )
...
def _build_payloads_for_changed(
    ...
    if len(jobs) <= 1 or index_workers <= 1:
        ...
        try:
            if index_parallel_backend == "thread":
                ...
                with ThreadPoolExecutor(max_workers=workers) as pool:
                    if hasattr(pool, "submit"):
                        future_map = {
                            pool.submit(_process_file_for_index, job): job
                            for job in jobs
                        }
                        ...
                        _log_deadlock_info(
                            "Executor missing submit(), falling back to map()"
                        )
                        results = []
                        for job, result in zip(
                            jobs, pool.map(_process_file_for_index, jobs), strict=False
                        ):
                            results.append(result)
                            _log_deadlock_info(f"Completed job: {job[1]}")
                            _progress(str(result.get("file", job[1])))
                ...
                with ProcessPoolExecutor(max_workers=workers) as process_pool:
                    if hasattr(process_pool, "submit"):
                        future_map = {
                            process_pool.submit(_process_file_for_index, job): job
                            for job in jobs
                        }
            ...
            _log_deadlock_info(
                f"Overall timeout after {overall_timeout}s, falling back to sequential"
            )
            ...
            for job in jobs:
                try:
                    ...
                    _log_deadlock_info(
                        f"Sequential processing error for {job[1]}: {exc!r}"
                    )
                    results.append(
                        {
                            "file": job[1],
            ...
            _log_deadlock_info(
                f"Parallel execution failed: {exc!r}, falling back to sequential"
            )
            # Fallback to sequential processing
            results = []
            for job in jobs:
    ...
    return {
        str(item.get("file", "")): item for item in results if str(item.get("file", ""))
    }
...
def _build_manifest_from_previous(
    ...
    pending_delta_ops = {
        kind: count_jsonl_lines(
            delta_path_for_base(base_path_for_kind(paths["index"], kind))
        )
...
def build_or_update_indexes(
    ...
    if full_rebuild:
        ...
        for rel_path, payload in changed_payloads.items():
            ...
            reference_ops.append(
                {"op": "set", "key": rel_path, "rows": references_rows}
            )
            dependency_ops.append(
                {"op": "set", "key": rel_path, "rows": dependencies_rows}
            )
        ...
        dependencies_added = append_delta_ops(
            paths["index"], "dependencies", dependency_ops
        )
    ...
    write_jsonl_atomic(
        base_path_for_kind(paths["index"], "test_ownership"), test_ownership
    )
