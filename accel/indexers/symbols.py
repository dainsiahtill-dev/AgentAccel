...
def _build_symbol_row(
    *,
    ...
    row: dict[str, Any] = {
        ...
        or f"{rel_path}:{str(symbol).strip()}",
        "scope": scope_value,
    ...
    *,
    ...
    if node_type in {"function_definition", "class_definition"}:
        ...
        if symbol:
            ...
            signature = _compact_signature_from_node_text(
                _node_text(node, source_bytes), "python"
            )
            relation_targets = (
                _extract_relation_targets_from_signature(signature)
    ...
    def visit(node: Any, scope_stack: list[tuple[str, str]]) -> None:
        ...
        if node_type in {
            ...
            if parent_kind == "class" and symbol_kind == "function":
                symbol_kind = "method"
            signature = _compact_signature_from_node_text(
                _node_text(node, source_bytes), lang
            )
