...
def search_symbols(
    ...
    *,
    ...
    if symbol_kinds:
        kind_filter = {
            k.lower().strip() for k in symbol_kinds
            if k.lower().strip() in _VALID_SYMBOL_KINDS
        }
        if not kind_filter:
            kind_filter = None
    ...
    if languages:
        lang_filter = {
            lang.lower().strip() for lang in languages
            if lang.lower().strip() in _VALID_LANGUAGES
        }
        if not lang_filter:
            lang_filter = None
...
def get_symbol_details(
    ...
    for row in all_symbols:
        ...
        if row_symbol.lower() == symbol_text.lower():
            candidates.append(row)
        elif row_qualified.lower() == symbol_text.lower():
            candidates.append(row)
        elif row_qualified.lower().endswith("." + symbol_text.lower()):
            candidates.append(row)
...
def _build_symbol_relations(
    ...
    for ref in all_refs:
        ...
        if target == symbol_name or target == qualified_name:
...
def build_call_graph(
    ...
    symbols_by_file = _group_by_file(
        load_index_rows(index_dir, kind="symbols", key_field="file"), "file"
    )
    references_by_file = _group_by_file(
        load_index_rows(index_dir, kind="references", key_field="file"), "file"
    )
    deps_by_file = _group_by_file(
        load_index_rows(index_dir, kind="dependencies", key_field="file"), "file"
    )
    ...
    if start_symbol_lower and not start_nodes:
        for file_path, rows in symbols_by_file.items():
            for row in rows:
                ...
                if symbol == start_symbol_lower or qualified == start_symbol_lower:
                    start_nodes.add(_normalize_path(file_path))
...
def get_symbol_context(
    ...
    if definition_file and context_type in {"definition", "related"}:
        definition_snippet = _read_file_snippet(
            project_dir, definition_file, line_start, line_end, context_lines=5
        )
