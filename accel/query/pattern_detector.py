...
def detect_patterns(
    ...
    if pattern_types:
        type_filter = {
            t.lower().strip() for t in pattern_types
            if t.lower().strip() in _VALID_PATTERN_TYPES
        }
        if not type_filter:
            type_filter = None
...
def _detect_singleton(classes: list[dict], functions: list[dict]) -> list[dict[str, Any]]:
    ...
    for func in functions:
        ...
        if scope:
            class_methods.setdefault(scope.lower(), []).append(
                str(func.get("symbol", "")).strip().lower()
            )
    ...
    for cls in classes:
        ...
        if isinstance(attributes, list):
            for attr in attributes:
                ...
                if "_instance" in attr_lower or "instance" == attr_lower:
                    confidence += 0.4
