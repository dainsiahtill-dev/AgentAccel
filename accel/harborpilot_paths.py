...
POLICY_URI = "policy://harborpilot/agent-spec/v3.0"
LEGACY_POLICY_URI = "policy://harborpilot/agent-spec/v2.11"
ACTIVE_RULES_URI = "harborpilot://policy/active_rules"
...
def resolve_harborpilot_paths(
    project_dir: Path | str | None = None,
) -> HarborPilotPaths:
    project_root = normalize_project_root(project_dir)
    hp_root = project_root / ".harborpilot"
    runtime_dir = hp_root / "runtime"
    runs_dir = runtime_dir / "policy_runs"
    return HarborPilotPaths(
        project_root=project_root,
        policy_path=project_root / "policy" / "harborpilot-agent-spec-v3.0.md",
        legacy_policy_path=project_root / "policy" / "harborpilot-agent-spec-v2.11.md",
        hp_root=hp_root,
...
def default_accel_runtime_home(project_dir: Path | str | None = None) -> Path:
    return resolve_harborpilot_paths(project_dir).accel_home
...
def resolve_artifact_path(
    ...
    *,
    ...
    paths = resolve_harborpilot_paths(project_dir)
