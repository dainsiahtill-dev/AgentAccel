...
class SessionReceiptStore:
    ...
    def _init_schema(self) -> None:
        ...
        try:
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_receipts_session_ts ON receipts(session_id, ts)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_receipts_run_ts ON receipts(run_id, ts)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_receipts_tool_status_ts ON receipts(tool, status, ts)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_receipt_events_job_ts ON receipt_events(job_id, ts)"
            )
        finally:
            conn.close()
    ...
    def attach_session(
        ...
        *,
        ...
        try:
            ...
            is_expired = (
                lease_until_dt is not None
                and str(row["lease_until"]).strip() != ""
    ...
    def list_receipts(
        ...
        *,
        ...
        sql = (
            ...
            + where_clause
            + " ORDER BY ts DESC, job_id DESC LIMIT ?"
        )
